...
1. 创建 TLS 证书文件。详情参见[操作说明](#create-tls-certificates)。
...
您可以按照以下步骤设置证书颁发机构（CA），或参阅[本指南](https://jamielinux.com/docs/openssl-certificate-authority/index.html)了解详细信息。
...
服务器端证书是使用相同的 CA 证书后，可以创建证书请求并使用 CA 对其进行签名。
...
此时你已经有证书，`broker.cert.pem`，和密钥，`broker.key-pk8.pem`。你可以使用证书和密钥以及 `ca.cert.pem` 来为代理和代理节点配置 TLS 传输加密。

# 使用手动生成的证书在 KoP 上启用TLS 

当不使用 cert-manager 在 KoP 上启用 TLS 时，首先要为 KoP 生成 SSL密钥和证书，然后为 KoP 证书和密钥创建 Kubernetes Secret。 
...
Kubernetes Secret 供你存储和管理敏感信息，如密码、token 和密钥。在 Secret 中存储机密信息比在 Pod 定义或容器镜像中逐字存储更加安全灵活。
